    ...
    FullTab: {  
        Create: { 
            ...
            getVal(resource){ 
                ...
                var data_facebook      = $(`${resource}`).find('.data-facebook').val(); 
                var data_google         = $(`${resource}`).find('.data-google').val(); 
                ...
                if (onPushData) {
                    ...
                    fd.append('data_facebook', data_facebook);
                    fd.append('data_google', data_google);
        ...
        Update: { 
            setVal(resource, data){
                ...
                $(`${resource}`).find('.data-google').val(data.google);
                $(`${resource}`).find('.data-facebook').val(data.facebook);
            },
            getVal(resource){ 
                var fd = new FormData();
                var required_data = [];
...
(() => {
    ...
    View.FullTab.onPush("Delete", "#popup-delete", () => {
        ...
        View.FullTab.default();
        IndexView.helper.showToastProcessing('Process', 'Đang xử lí');
        Api.Community.Delete($("#popup-delete").find('.data-id').val())
            .done(res => { 
                IndexView.helper.showToastSuccess('Success', 'Thành công');
                getData();
            })
